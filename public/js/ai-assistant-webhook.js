class AIAssistantForm {
    constructor(formId) {
        ...
        this.initWebhookUrl().then(() => {
            // Entfernt: console.log('Webhook URL initialized:', this.webhookUrl);
        }).catch(error => {
    ...
    async fetchLoadingFacts() {
        try {
            ...
            // Entfernt: console.log('Loading facts fetched successfully:', this.loadingFacts);
        } catch (error) {
    ...
    handleWebhookResponse(result) {
        // Entfernt: console.log('Handling webhook response:', result);
...
// Entfernt: console.log('AIAssistantForm class defined');
